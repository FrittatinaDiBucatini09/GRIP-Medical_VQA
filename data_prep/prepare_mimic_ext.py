...
def parse_args():
    ...
    parser.add_argument('--keep_empty_answers', action='store_true',
                       help='Keep samples with empty answers (default: filter them out)')
...
def main():
    ...
    with open(OUTPUT_CSV, 'w', newline='', encoding='utf-8') as f:
        ...
        for item in tqdm(data, desc="Processing"):
            ...
            answer_text = ans_list[0] if isinstance(ans_list, list) and ans_list else ""

            # 5. Filter out empty answers (default behavior)
            if not answer_text and not args.keep_empty_answers:
    ...
    if empty_answer_count > 0:
        print(f"[INFO] Filtered out {empty_answer_count} samples with empty answers.")
    if skipped_count > 0:
