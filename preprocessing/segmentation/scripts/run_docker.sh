...
# If the config references any SAM3-family model (Joey MedSAM3 or AIM
# Medical-SAM3), use the SAM3 image (CUDA 12.6+). Otherwise fall back to the
# default 3090 image (CUDA 12.2).
_CONF2_HOST="$PHYS_DIR/configs/${CONF_STEP2#/workspace/configs/}"
if [ -f "$_CONF2_HOST" ] && grep -qE "medsam3|medical_sam3" "$_CONF2_HOST" 2>/dev/null; then
    IMAGE_NAME="pipeline_segmentation:sam3"
    echo "[INFO] Detected SAM3-family config → using Docker image: $IMAGE_NAME"
else
...
docker run --rm \
    ...
    -e WANDB_DATA_DIR="/workspace/data/results/cache/wandb" \
