...
# Medical-SAM3 (AIM-Research-Lab) — full fine-tune of SAM3, distributed as a
# single .pt checkpoint on HuggingFace. Loaded via the SAM3Model wrapper from
# Medical-SAM3/inference/sam3_inference.py (no LoRA layers).
MEDICAL_SAM3_AIM_MODELS = {
    "medical_sam3": "checkpoint.pt",
}
MEDICAL_SAM3_AIM_HF_REPO = "Chongcong/Medical-SAM3"

# Combined lookup for quick validation
ALL_MODEL_KEYS = (
    set(SAM1_MODELS.keys())
    | set(MEDSAM2_MODELS.keys())
    | set(MEDSAM3_MODELS.keys())
    | set(MEDICAL_SAM3_AIM_MODELS.keys())
)
...
def is_medsam3_key(key: str) -> bool:
    """Returns True if the key refers to a MedSAM3 (Joey LoRA) model."""
    return key in MEDSAM3_MODELS

def is_medical_sam3_aim_key(key: str) -> bool:
    """Returns True if the key refers to AIM-Research-Lab Medical-SAM3."""
    return key in MEDICAL_SAM3_AIM_MODELS
...
def download_medical_sam3_model(model_name: str, checkpoint_dir: str) -> str:
    """Downloads AIM-Research-Lab Medical-SAM3 weights from HuggingFace Hub."""
    filename = MEDICAL_SAM3_AIM_MODELS.get(model_name)
    if not filename:
        raise ValueError(
            f"Unknown Medical-SAM3 (AIM) model: {model_name}. "
            f"Available: {list(MEDICAL_SAM3_AIM_MODELS.keys())}"
        )

    os.makedirs(checkpoint_dir, exist_ok=True)
    # Namespace under a subdir to avoid colliding with Joey's checkpoint.pt-likes
    target_dir = os.path.join(checkpoint_dir, "medical_sam3_aim")
    ...
    print(f"[INFO] Downloading {model_name} from HuggingFace ({MEDICAL_SAM3_AIM_HF_REPO})...")

    try:
        from huggingface_hub import hf_hub_download
    except ImportError:
        raise ImportError("huggingface_hub is required for Medical-SAM3 downloads. pip install huggingface_hub")

    hf_hub_download(
        repo_id=MEDICAL_SAM3_AIM_HF_REPO,
        filename=filename,
...
def download_model(model_name: str, checkpoint_dir: str) -> str:
    ...
    elif model_name in MEDICAL_SAM3_AIM_MODELS:
        return download_medical_sam3_model(model_name, checkpoint_dir)
    else:
...
def load_medical_sam3_model(checkpoint_path: str, device: str = "cuda",
                            checkpoint_dir: str = "/workspace/checkpoints",
                            confidence_threshold: float = 0.1,
                            sam3_root: Optional[str] = None):
    """
    Loads AIM-Research-Lab Medical-SAM3 (full fine-tune of SAM3).
    Accepts a file path OR a known model key ("medical_sam3").
    Returns a SAM3Model instance from Medical-SAM3/inference/sam3_inference.py.

    The wrapper exposes:
        - encode_image(np.ndarray RGB) → inference_state
        - predict_box(inference_state, (x1,y1,x2,y2), (H,W)) → uint8 mask | None
        - predict_text(inference_state, text_prompt)         → uint8 mask | None
        - .processor → underlying Sam3Processor (for combined text+box prompts)
    """
    try:
        # /opt/Medical-SAM3/inference is on PYTHONPATH (set in Dockerfile.sam3)
        import sam3_inference as _aim_mod
    except ImportError:
        raise ImportError(
            "Medical-SAM3 (AIM) module not found. The Dockerfile.sam3 image must "
            "clone https://github.com/AIM-Research-Lab/Medical-SAM3 to "
            "/opt/Medical-SAM3 and place /opt/Medical-SAM3/inference on PYTHONPATH."
        )

    # The upstream module hardcodes SAM3_ROOT = Path("../sam3") and then loads
    # the BPE asset as `SAM3_ROOT / "assets" / "bpe_simple_vocab_16e6.txt.gz"`.
    # In our container two `sam3` package roots coexist:
    #   /opt/sam3/sam3              ← real facebookresearch/sam3 (pip -e)
    #   /opt/MedSAM3/sam3           ← Joey-S-Liu's bundled fork (PYTHONPATH wins)
    # In both, the BPE actually lives inside the *package* dir's assets/, not
    # under the repo root. We derive SAM3_ROOT from whichever `sam3` actually
    # imports, so the BPE lookup always matches the running model_builder.
    from pathlib import Path as _PPath
    if sam3_root is not None:
        sam3_root_path = _PPath(sam3_root)
    else:
        try:
            import sam3 as _sam3_pkg
        except ImportError as _e:
            raise ImportError(
                f"Cannot import the `sam3` package needed by Medical-SAM3 (AIM): {_e}"
            )
        sam3_root_path = _PPath(_sam3_pkg.__file__).resolve().parent

    bpe_asset = sam3_root_path / "assets" / "bpe_simple_vocab_16e6.txt.gz"
    if not bpe_asset.is_file():
        raise FileNotFoundError(
            f"BPE asset not found at {bpe_asset}. Resolved sam3 package root: "
            f"{sam3_root_path}. Verify Dockerfile.sam3 cloned facebookresearch/sam3 "
            f"and Joey-S-Liu/MedSAM3 with their `assets/` subdirectories intact."
        )
    print(f"[INFO] Medical-SAM3 (AIM): SAM3_ROOT → {sam3_root_path} (bpe={bpe_asset.name})")
    _aim_mod.SAM3_ROOT = sam3_root_path

    # Resolve key → path
    if checkpoint_path in MEDICAL_SAM3_AIM_MODELS:
        print(f"[INFO] '{checkpoint_path}' is a known Medical-SAM3 (AIM) key. Checking cache...")
        checkpoint_path = download_medical_sam3_model(checkpoint_path, checkpoint_dir)

    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"Medical-SAM3 (AIM) checkpoint not found at {checkpoint_path}")

    print(f"[INFO] Loading Medical-SAM3 (AIM) from {checkpoint_path} (sam3_root={sam3_root})")
    predictor = _aim_mod.SAM3Model(
        confidence_threshold=confidence_threshold,
        device=device,
        checkpoint_path=checkpoint_path,
    )
    # Force eager weight load so the first inference doesn't pay the cost
    # silently and so we surface load errors immediately at startup.
...
def load_model(checkpoint_path: str, device: str = "cuda",
    ...
    """
    Unified model loader. Automatically detects family.
    Returns (predictor, model_family) where model_family is one of:
        'sam1', 'sam2', 'sam3' (Joey LoRA), 'sam3_aim' (AIM full fine-tune).
    """
    # Medical-SAM3 (AIM) key — checked before generic 'sam3' path inference
    if checkpoint_path in MEDICAL_SAM3_AIM_MODELS:
        predictor = load_medical_sam3_model(checkpoint_path, device, checkpoint_dir)
        return predictor, "sam3_aim"

    # MedSAM3 (Joey LoRA) key
    if checkpoint_path in MEDSAM3_MODELS:
    ...
    if "medical_sam3" in lower_path or "medical-sam3" in lower_path or "aim" in lower_path:
        predictor = load_medical_sam3_model(checkpoint_path, device, checkpoint_dir)
        return predictor, "sam3_aim"
    elif "medsam3" in lower_path or "sam3" in lower_path or "lora" in lower_path:
        predictor = load_medsam3_model(checkpoint_path, device, checkpoint_dir, sam3_cfg)
        return predictor, "sam3"
