...
def _generate_cache_check_block(
    ...
    # Build the cache command as a function so quoting is handled once.
    # The config-file arg must be a single quoted value even when it contains
    # space-separated paths (segmentation step1 + step2).
    return f"""
...
CACHE_DATASET="${{DATA_FILE_OVERRIDE:-default}}"

# Helper: invoke cache_utils.py with consistent quoting
_run_cache_util() {{
    local subcmd="$1"
    local extra_args=()
    [ -n "$CACHE_CONFIG_FILE" ] && extra_args+=(--config-file "$CACHE_CONFIG_FILE")
    [ "$NER_ENABLED" = "true" ] && extra_args+=(--ner-enabled)
    python3 "$CACHE_UTIL" "$subcmd" \\
        --results-dir "$CACHE_RESULTS_DIR" \\
        --dataset "$CACHE_DATASET" \\
        "${{extra_args[@]}}"
}}

CACHE_OUTPUT=$(_run_cache_util check 2>&1) && CACHE_HIT=true || CACHE_HIT=false
echo "$CACHE_OUTPUT"
...
if [ "$CACHE_HIT" = "true" ]; then
    ...
    if echo "$CACHE_OUTPUT" | grep -q "NER configuration changed"; then
        ...
        if rm -rf "$CACHE_RESULTS_DIR"/* 2>/dev/null; then
            echo "[CACHE] Stale results cleared."
        else
            echo "[CACHE] Standard rm failed (likely Docker UID mismatch). Attempting cleanup via Docker..."
            if [ -d "$CACHE_RESULTS_DIR" ]; then
                ABS_RESULTS_DIR="$(cd "$CACHE_RESULTS_DIR" && pwd)"
                if docker run --rm -v "$ABS_RESULTS_DIR":/clean_target -w /clean_target alpine sh -c "rm -rf /clean_target/*"; then
                    echo "[CACHE] Docker cleanup succeeded."
                else
                    echo "[CACHE] ERROR: Docker cleanup also failed. Aborting."
                    exit 1
                fi
            fi
        fi
