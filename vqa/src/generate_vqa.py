...
# Prompt prefix injected when images have been preprocessed by a specific stage.
PREPROC_CONTEXT = {
    "attn_map": (
...
def prepare_prompts(df: pd.DataFrame, use_few_shot: bool = False,
    ...
    for _, row in df.iterrows():
        formatted_question = format_question_text(row['question'])
...
def sanitize_for_rouge(text: str) -> str:
    ...
    # Remove XML-style tags (<answer>, <think>, etc.)
    s = re.sub(r'</?[a-zA-Z][a-zA-Z0-9_]*\s*/?>', '', s)

    # Remove markdown bold/italic wrappers around "Answer:" prefixes
    s = re.sub(
    ...
    # Strip common instruction-tuned model boilerplate preambles
    s = re.sub(
...
def calculate_metrics(y_true_answers: List[str], y_pred_answers: List[str]) -> Dict:
    ...
    for true_answer, pred_answer in zip(y_true_answers, y_pred_answers):
        ...
        if true_sanitized and pred_sanitized:
...
def filter_missing_images(df: pd.DataFrame, image_column: str) -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Verifying images"):
        ...
        # Non-string sources (bytes, HuggingFace Image objects) are checked lazily by load_image
...
def prepare_chat_conversations(df: pd.DataFrame, use_cot: bool = False, use_images: bool = False,
    ...
    # Sample few-shot examples from a separate split when available to avoid data leakage
    few_shot_examples = ""
    if use_few_shot:
    ...
    for idx, row in df.iterrows():
        ...
        formatted_question = format_question_text(row[question_column])
        ...
        if use_images:
            if images_dir:
                ...
                if isinstance(image_source, str):
                    # Resolve relative paths against BASE_IMAGE_PATH so vLLM can
                    # find them under allowed_local_media_path.
            ...
            if image_path:
                # vLLM requires "image_url" type with a file:// prefixed absolute path
                content = [
                ...
                conversation = [{"role": "user", "content": user_message_text}]
        else:
            conversation = [{"role": "user", "content": user_message_text}]
...
def main():
    ...
    if args.use_images:
        # Restrict vLLM file access to BASE_IMAGE_PATH (safer than allowing "/")
    ...
    try:
        ...
        # OctoMed uses <think>...</think> reasoning blocks — don't truncate with stop tokens
